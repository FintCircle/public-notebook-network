...
function NotepageAbout() {
  ...
  return (
    ...
    >
      <div className="mx-auto min-w-0 max-w-2xl overflow-hidden">
        ...
        <header className="mt-12 min-w-0 border-b border-current/15 pb-8 sm:mt-16 sm:pb-10">
          <img
          ...
          {isEditing ? (
            <div className="mt-5 grid gap-3">
              <input value={pageTitle} onChange={(event) => setPageTitle(event.target.value)} aria-label="About page title" className="min-w-0 w-full border-b border-current/20 bg-transparent py-1 font-heading text-4xl tracking-tight outline-none focus:border-current/50 sm:text-5xl" />
              <textarea value={intro} onChange={(event) => setIntro(event.target.value)} aria-label="About page introduction" rows={2} className="w-full max-w-[42ch] resize-y border border-current/20 bg-transparent p-2 text-base leading-relaxed outline-none focus:border-current/50 sm:text-lg" />
            </div>
          ) : (
            <>
        ...
        <section className="grid min-w-0 gap-8 py-8 sm:gap-10 sm:py-10 sm:grid-cols-[1fr_0.7fr]" aria-label="About details">
          {isEditing && (
            <div>
              <label htmlFor="about-bio-editor" className="text-sm opacity-55">About content</label>
              <input ref={imageInputRef} type="file" accept="image/*" onChange={addBioImage} className="sr-only" />
              <div className="mt-3 flex flex-wrap gap-1 border border-current/20 p-1" role="toolbar" aria-label="About formatting">
                ...
                <button type="button" onMouseDown={(event) => event.preventDefault()} onClick={() => imageInputRef.current?.click()} className="px-2 py-1 text-xs hover:bg-current/10">Image</button>
              </div>
              <div
                id="about-bio-editor"
                ref={editorRef}
