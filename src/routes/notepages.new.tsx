...
function NewNotepage() {
  ...
  return (
    <div className="min-h-screen"><SiteNav /><main className="mx-auto max-w-2xl px-5 py-14">
      ...
      {step === 2 && <section className="mt-5"><h1 className="text-3xl">Make it yours</h1><p className="hand mt-2 text-xl opacity-60">choose its cover and type</p><div className="mt-9"><p className="text-sm">Cover image</p><label className="mt-3 flex min-h-28 cursor-pointer flex-col items-center justify-center gap-2 rounded-md border border-dashed border-input bg-muted/35 px-5 text-center text-sm transition-colors hover:bg-muted"><ImagePlus aria-hidden className="size-5" /><span>{coverUrl ? "Choose a different image" : "Choose an image"}</span><input type="file" accept="image/*" className="sr-only" onChange={(event) => chooseCover(event.target.files?.[0])} /></label></div><div className="mt-8"><p className="text-sm">Background</p><div className="mt-3 grid grid-cols-3 gap-3">{backgrounds.map((option, index) => <Button key={option.name} variant="outline" onClick={() => setBackground(index)} className={`h-20 whitespace-normal ${option.className} ${background === index ? "ring-2 ring-ring" : ""}`}><span>{option.name}</span>{background === index && <Check aria-hidden />}</Button>)}</div></div><div className="mt-8"><p className="text-sm">Type</p><div className="mt-3 grid gap-2">{types.map((font) => <Button key={font} variant="outline" onClick={() => setType(font)} className="h-12 justify-between" aria-pressed={type === font}><span style={{ fontFamily: font }}>{font}</span>{type === font && <Check aria-hidden />}</Button>)}</div></div><div className={`relative mt-9 flex aspect-[3/4] items-end overflow-hidden p-6 ${selectedBackground.className}`}>{coverUrl && <img src={coverUrl} alt="Your selected cover preview" className="absolute inset-0 size-full object-cover" />}<div aria-hidden className={coverUrl ? "notepage-cover-shade absolute inset-0" : "hidden"} /><div className={`relative z-10 ${coverUrl ? "text-[var(--np-cover-ink)]" : ""}`}><p className="text-xs uppercase opacity-60">Cover preview</p><h2 className="mt-5 max-w-[10ch] text-4xl leading-none" style={{ fontFamily: type }}>{name || "Your Notepage"}</h2><p className="mt-4 max-w-[30ch] text-sm leading-relaxed opacity-80">{description || "Your description will live here."}</p></div></div><div className="mt-8 flex gap-3"><Button variant="ghost" onClick={() => setStep(1)}><ChevronLeft aria-hidden />Back</Button><Button onClick={() => setStep(3)}>Create Notepage <ChevronRight aria-hidden /></Button></div></section>}
