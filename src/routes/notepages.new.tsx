...
function NewNotepage() {
  ...
  return (
    <div className="min-h-screen"><SiteNav /><main className="mx-auto max-w-2xl px-5 py-14">
      ...
      {step === 2 && <section className="mt-5"><h1 className="text-3xl">Make it yours</h1><p className="hand mt-2 text-xl opacity-60">quiet choices are still choices</p><div className="mt-9"><p className="text-sm">Background</p><div className="mt-3 grid grid-cols-3 gap-3">{backgrounds.map((option, index) => <Button key={option.name} variant="outline" onClick={() => setBackground(index)} className={`h-20 whitespace-normal ${option.className} ${background === index ? "ring-2 ring-ring" : ""}`}><span>{option.name}</span>{background === index && <Check aria-hidden />}</Button>)}</div></div><div className="mt-8"><p className="text-sm">Type</p><div className="mt-3 grid gap-2">{types.map((font) => <Button key={font} variant="outline" onClick={() => setType(font)} className="h-12 justify-between" aria-pressed={type === font}><span style={{ fontFamily: font }}>{font}</span>{type === font && <Check aria-hidden />}</Button>)}</div></div><div className={`mt-9 border-y border-current/10 p-6 ${backgrounds[background].className}`}><p className="text-xs uppercase opacity-45">Preview</p><h2 className="mt-5 text-2xl" style={{ fontFamily: type }}>{name || "Your Notepage"}</h2><p className="mt-3 text-sm leading-relaxed opacity-70">{description || "Your description will live here."}</p></div><div className="mt-8 flex gap-3"><Button variant="ghost" onClick={() => setStep(1)}><ChevronLeft aria-hidden />Back</Button><Button onClick={() => setStep(3)}>Create Notepage <ChevronRight aria-hidden /></Button></div></section>}
